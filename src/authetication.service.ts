...
import {
  ...
  UserCredential // Importar UserCredential
...
export class AutheticationService {
  ...
  async register(email: string, password: string): Promise<UserCredential> { // Cambiado el tipo de retorno
    try {
      const userCredential = await createUserWithEmailAndPassword(this.auth, email, password);
      // Devuelve userCredential para que podamos acceder al UID
  ...
  // --- NUEVO MÉTODO PARA GUARDAR USUARIO EN FIRESTORE ---
  ...
  async login(email: string, password: string): Promise<UserCredential> { // Cambiado el tipo de retorno
    try {
